   ...
   /* public function signup(Request $request)
    ...
    public function user_post(Request $request)
    {
     ...
     public function getPostReactions(Request $request, $postId)
     {
         ...
         if($postId == "") $postId = $request->postId;
    ...
    public function create_story(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            if($request->description == "" && $request->content_type == "") return response()->json(['error' => 'Story description or files not added. Enter the correct data.'], 404);
            ...
            if ($request->content_type == 'text') {
                ...
                if (!empty($request->description)) {
                    $data['description'] = json_encode(
                        array('color' => $request->color, 'bg-color' => $request->bg_color, 'text' => $all_data['description'])
                    );
                }
            }
            ...
            $response['success'] = true;
            $response['message'] = 'Story uploaded';
    ...
    public function create_post(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            if ($post_id >= 0) {
                $response['status'] = 200;
                $response['message'] = 'Your post successfully published';
            }
    ...
    public function edit_post($id, Request $request)
    {
        ...
        $response = array();
        $data = array();
        if(!isset($id)) $id = $request->id;

       // echo '2222';exit();
        if (isset($token) && $token != '') {
            $user_id = auth('sanctum')->user()->id;

            $posts = Posts::where('post_id', $id)->first();
            if (!$posts) {
                return response()->json(['error' => 'Post not found'], 404);
            }
    ...
    {  //echo '222';exit();
        ...
        if (isset($token) && $token != '') {
            ...
            if ($done) {
                $response['status'] = 200;
                $response['message'] = 'Your post successfully deleted';
            }
        }
    ...
    public function save_post_report(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $done = $report->save();
            if ($done) {
                return response()->json(['message' => 'Post Report saved successfully']);
            } else {
                return response()->json(['error' => 'Something is error'], 404);
            }
        }
        return $response;
    }

    public function get_media_file($id, Request $request)
    {
        $token = $request->bearerToken();
        $response = array();
    ...
    public function post_media_file($id, Request $request)
    {
        ...
        if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
            $fileType = 'image';
            $file->storeAs('public/storage/post/images/', $fileName);
        } elseif (in_array($extension, ['mp4', 'mov', 'avi', 'webm'])) {
            $fileType = 'video';
            $file->storeAs('public/storage/post/videos/', $fileName);
        }else{
            $file->storeAs('public/storage/post/images/', $fileName);
        }
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'file_url' => asset('public/storage/post/images/' . $media->file_name),
            ]
        ]);
    }
    ...
    function profile_photos(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $response['all_photos'] = $photoArray;
            ...
            $response['all_albums'] = $albumArray;
            ...
            $response['all_videos'] = $videoArray;
    ...
    public function comment_delete(Request $request, $comment_id)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            if ($done) {
                $response = array('message' => get_phrase('Comment' .$comment_id.' Deleted Successfully'), 'success' => true);
            }
        }
        return response()->json($response);
    }
    ...
    public function create_group(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            // Store file in storage/app/public/groups
            if ($request->image && !empty($request->image)) {
                //$file_name = FileUploader::upload($request->image, 'public/groups/logo', 300);
                $file = $request->file('image');
                $filename = uniqid() . '.' . $file->getClientOriginalExtension();
                $path = $file->storeAs('public/groups/logo', $filename);
                $file_url = asset('storage/groups/logo/' . $filename);
            }
    ...
    public function groups_join_remove(Request $request, $id)
    {
        ...
        if (isset($token) && $token != '') {
            try {
                ...
                $member = Group_member::where('user_id', $user_id)
                    ->where('group_id', $id)
                    ->first();
                ...
                if ($member) {
                    ...
                    $response['success'] = true;
                    $response['message'] = 'User removed from the group successfully.';
                } else {
                    $response['success'] = false;
                ...
                $response['success'] = false;
                $response['message'] = 'Error removing user from the group: ' . $e->getMessage();
            }
        } else {
            $response['success'] = false;
            $response['message'] = 'Invalid token.';
    ...
    public function view_videos(Request $request, $id)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $video = Video::find($id);print_r($video);
            if(empty($video) || $video == null) {
                return response()->json(['error' => 'Video does not exist'], 404);
            }
    ...
    public function create_videos(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $file = $request->file('file');
            $mobile_image = $request->file('mobile_app_image');
            ...
            $file->storeAs('public/storage/videos', $file_name);

            $extension = strtolower($mobile_image->getClientOriginalExtension());
            $mobile_app_image = Str::uuid() . '.' . $extension;
            $mobile_image->storeAs('public/storage/videos', $mobile_app_image);
        ...
        if (isset($token) && $token != '') {
            ...
            if ($video) {
                ...
                if ($done) {
                    ...
                    $response = array(
                        'id' => $id,
                        'message' => get_phrase('Video Deleted Successfully'),
                    );
                } else {
                    // If deletion fails
                    $response = array(
                        'id' => $id,
                        'message' => get_phrase('Failed to delete Video'),);
                }
            } else {
                // If the product with the given ID is not found
                $response = array(
                    'id' => $id,
                    'message' => get_phrase('Video not found'),
                );
            }
        } else {
            $response['success'] = false;
    ...
    public function update_jobs(Request $request, $id)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $job = Job::find($id);
            if($job == null) return response()->json(['error' => 'Job not found'], 404);
    ...
    public function JobApply(Request $request, $id)
    {
        ...
        if (isset($token) && $token != '') {
            ...
            $request->validate([
                'email' => 'required',
                'phone' => 'required',
                'image' => 'file|mimes:pdf,jpeg,jpg,png,gif|max:10240',
            ]);
            //echo '2222'; exit();
            ...
            if($owner_id == null) return response()->json("Owner or job does not exist", 404);
            $apply = new JobApply();
