...
class UserController extends Controller
{
    ...
    public function ads(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            $response['ads'] = Sponsor::where('user_id', $user_id)->get();
            $response['view_path'] = 'ads';
        }
    ...
    public function ad_store(Request $request)
    {
        ...
        if (isset($token) && $token != '') {
            try{
                ...
                $response['ext_url'] = $request->ext_url;
                $response['image'] = random(40) . '.' . $request->image->extension();

                Sponsor::insert($response);
                FileUploader::upload($request->image, 'public/storage/sponsor/thumbnail/' . $response['image'], 300);

                $response['success'] = true;
                $response['message'] = 'New ad created successfully';

                return response()->json($response);
            } catch (ModelNotFoundException $e) {
                return response()->json(['error' => 'Error while uploading'], 404);
            }
        }else {
    ...
    {
        $response['ad'] = Sponsor::where('id', $id)->where('user_id', auth()->user()->id)->first();
        return response()->json($response);
    }
